...
#
def print_folded_html_report(covdata, output_file, options):
    ...
    commondirs = list(set(sorted([os.path.split(x)[0] for x in files])))
    common_dir_depth_two = list(
        set(list(map(lambda x: os.sep.join(x.split("/")[:2]), commondirs)))
    )
    common_dir_depth_two = list(
    ...
    dir_coverage = {}
    for com in common_dir_depth_two:
        dir_coverage[com] = {
            "lines_total": 0,
            "lines_hits": 0,
            "lines_coverage": 0.0,
    ...
    for f in keys:
        ...
        lines_covered = calculate_coverage(class_hits, class_lines, nan_value=None)
        ...
        for com in common_dir_depth_two:
            if com in f:
                dir_coverage[com]["lines_total"] += class_lines
                dir_coverage[com]["lines_hits"] += class_hits
                dir_coverage[com]["branches_total"] += b_total
                dir_coverage[com]["branches_hits"] += b_hits
                has_parent = True

        nrows += 1
        fname = os.path.relpath(os.path.realpath(cdata_fname[f]), data["DIRECTORY"])
    ...
    for com in common_dir_depth_two:
        nrows += 1
        lines_coverage = calculate_coverage(
            dir_coverage[com]["lines_hits"],
            dir_coverage[com]["lines_total"],
            nan_value=0.0,
        )
        branches_coverage = calculate_coverage(
            dir_coverage[com]["branches_hits"],
        ...
        data["ROWS"].append(
            html_row(
                ...
                directory=data["DIRECTORY"],
                filename=sourcefile,
                LinesExec=dir_coverage[com]["lines_hits"],
                LinesTotal=dir_coverage[com]["lines_total"],
                LinesCoverage=lines_coverage,
                BranchesExec=dir_coverage[com]["branches_hits"],
                BranchesTotal=dir_coverage[com]["branches_total"],
                BranchesCoverage=branches_coverage,
                IsSourceFile=False,
    ...
    if output_file is None:
        ...
        with io.open(
            output_file, "w", encoding=options.html_encoding, errors="xmlcharrefreplace"
        ) as fh:
            fh.write(htmlString + "\n")
    ...
    for f in keys:
        ...
        with io.open(
            data["FILENAME"], "r", encoding=options.source_encoding, errors="replace"
        ) as INPUT:
...
def html_row(options, details, sourcefile, nrows, **kwargs):
    ...
    if nrows % 2 == 0:
        ...
        kwargs["altstyle"] = ""
    if details and kwargs["IsSourceFile"]:
        kwargs["filename"] = '<a href="%s">%s</a>' % (
            sourcefile,
            kwargs["filename"].replace("\\", "/"),
        )
    ...
    kwargs["BranchesColor"] = coverage_to_color(
        kwargs["BranchesCoverage"],
        options.html_medium_threshold,
        options.html_high_threshold,
    )
    kwargs["BranchesCoverage"] = (
        "-"
        if kwargs["BranchesCoverage"] is None
        else round(kwargs["BranchesCoverage"], 1)
    )
...
def _make_short_sourcename(output_file, filename):
    ...
    longname_hash = ""
    while True:
        sourcename = ".".join((output_prefix, longname + longname_hash, output_suffix))
